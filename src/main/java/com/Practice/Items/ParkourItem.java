...
public class ParkourItem implements DefaultPPPItem {
    ...
    public boolean add(ItemHandler.NewItem newItem) {
        ...
        newItem.player.sendMessage(ChatColor.LIGHT_PURPLE + "You have added a " + ChatColor.GOLD + "Parkour Item" + ChatColor.LIGHT_PURPLE + " item, Sheesh.");
        return true;
    }
    ...
    public ItemStack create(Material item, String itemName, int count, int[] coords, String world) {
        ...
        tag.setInt("PPPType", ItemType.PARKOUR.id);
    ...
    public void run(Player player, ItemStack clickedItem) {
        ...
        Location loc = new Location(world, c[0], c[1], c[2]);

        ParkourGame.addPlayer(player, loc);
    }
